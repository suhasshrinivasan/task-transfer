...
def build_flow_model(
    ...
    # for normal base distributions
    if "_lrmn" in flow_base_distribution:  # _lrmn denotes "learn mean"
        loc = torch.nn.Parameter(torch.zeros(dims))
    else:
        loc = torch.zeros(dims)

    flow_base_distribution = flow_base_distribution.replace("_lrmn", "")
    if flow_base_distribution == "normal":
        flow_base_distribution = G.IndependentNormal(loc=loc, scale=torch.ones(dims))
