...
class OrientationDiscriminationModel:
    def __init__(
        ...
        self.x_feedback_factor = x_feedback_factor
    ...
    def sample_prior(self, n_samples=1):
        ...
        n_g = len(self.g_pref_orientations)
        g_probs = (
            2
            * torch.pi
            / n_g
            * cos2_von_mises(
                x=diff_g_orientations, loc=0.0, concentration=self.g_tuning_specificity
            )
        )
        g_samples = D.Bernoulli(probs=g_probs).sample()
        x_scale_factor = cos2_von_mises(
            self.g_pref_orientations.unsqueeze(-1)
            - self.x_pref_orientations.unsqueeze(0),
            loc=0.0,
            concentration=self.x_tuning_specificity,
        )
        x_scale = 1 + self.x_feedback_factor * g_samples @ x_scale_factor
